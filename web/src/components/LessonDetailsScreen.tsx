...
import { ArrowLeft, BookOpen, BrainCircuit, FileText, Sparkles, MessageCircle, Lightbulb, CheckCircle2, RotateCcw, Clock, ChevronRight, HelpCircle, Eye, EyeOff, BookOpenCheck, FileQuestion, ListChecks } from 'lucide-react';
...
import { ImportantQuestion, MCQQuestion, MCQDifficulty } from '@smartstudy/shared';
...
export default function LessonDetailsScreen() {
  ...
  const [result, setResult] = useState<{ type: 'summary' | 'quiz' | 'flashcards' | 'topics' | 'explanation' | 'important-questions' | 'mcq-generator', data: any } | null>(null);
  const [qaError, setQaError] = useState<string | null>(null);
  const [expandedQuestions, setExpandedQuestions] = useState<Record<string, boolean>>({});
  const [viewInlinePdf, setViewInlinePdf] = useState<boolean>(false);

  // Dedicated states for standalone MCQ Generator
  const [mcqCount, setMcqCount] = useState<number>(10);
  const [mcqDifficulty, setMcqDifficulty] = useState<MCQDifficulty>('medium');
  const [mcqSubState, setMcqSubState] = useState<'config' | 'active' | 'result'>('config');
  const [mcqQuestions, setMcqQuestions] = useState<MCQQuestion[]>([]);
  const [mcqAnswers, setMcqAnswers] = useState<Record<number, number>>({});
  const [mcqCurrentIndex, setMcqCurrentIndex] = useState<number>(0);
  ...
  const handleAction = async (type: 'summary' | 'quiz' | 'flashcards' | 'topics' | 'explanation' | 'important-questions' | 'mcq-generator') => {
    setLoading(type);
    setQaError(null);
    try {
      if (type === 'summary') {
        ...
        setResult({ type, data: [] });
        const questions = await geminiService.generateImportantQuestions(lesson.content);
        setExpandedQuestions({});
        setResult({ type, data: questions });
      } else if (type === 'mcq-generator') {
        setResult({ type, data: null });
        setMcqSubState('config');
      }
    } catch (e: any) {
      console.error(e);
      setQaError(e?.message || 'Failed to generate content. Please try again.');
      if (type === 'important-questions' || type === 'mcq-generator') {
        setResult({ type, data: [] });
      }
    } finally {
      setLoading(null);
  ...
  const handleGenerateMCQs = async () => {
    setLoading('mcq-generator');
    setQaError(null);
    try {
      if (!lesson.content || !lesson.content.trim()) {
        throw new Error('Lesson content is empty. Cannot generate MCQs.');
      }
      const questions = await geminiService.generateMCQs(lesson.content, mcqCount, mcqDifficulty);
      ...
      setMcqSubState('active');
      setResult({ type: 'mcq-generator', data: questions });
    } catch (e: any) {
      console.error(e);
      setQaError(e?.message || 'Failed to generate MCQs. Please try again.');
      setResult({ type: 'mcq-generator', data: [] });
    } finally {
      setLoading(null);
    }
  };
        ...
        <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm sticky top-24">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <ActionButton 
              ...
              loading={loading === 'important-questions'}
              color="text-purple-600 bg-purple-50 hover:bg-purple-100"
            />
            <ActionButton 
              icon={ListChecks} 
              label="MCQ Generator" 
              onClick={() => handleAction('mcq-generator')} 
              loading={loading === 'mcq-generator'}
        ...
        <AnimatePresence>
          {result && (
            ...
            >
              ...
              {result.type === 'important-questions' && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <h3 className="font-display font-bold text-xl flex items-center gap-2 text-slate-900">
                    ...
                    <button
                      onClick={() => handleAction('important-questions')}
                      disabled={loading === 'important-questions'}
                      className="px-3 py-1 bg-purple-50 hover:bg-purple-100 text-purple-700 font-bold text-xs rounded-xl border border-purple-200 transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50"
                    >
                      <RotateCcw className="w-3.5 h-3.5" /> Generate Again
                    </button>
                  </div>

                  {qaError && (
                    <div className="p-4 bg-rose-50 border border-rose-100 rounded-2xl flex items-center justify-between text-rose-600 text-xs font-medium">
                      <span>{qaError}</span>
                      <button
                        onClick={() => handleAction('important-questions')}
                  ...
                  {(!result.data || result.data.length === 0) ? (
                    <div className="p-8 text-center bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
                      <p className="font-bold text-slate-700 text-sm">No exam questions generated</p>
                      <p className="text-xs text-slate-400">Click below to generate high-yield questions from this lesson.</p>
                      <button
                        onClick={() => handleAction('important-questions')}
                        className="mt-3 px-5 py-2.5 bg-purple-600 text-white font-bold text-xs rounded-xl hover:bg-purple-700 transition-all cursor-pointer"
                      >
                        Generate Questions
                      </button>
                    </div>
                    ...
                    <div className="space-y-8">
                      {[
                        { marks: 1, label: '1 MARK — Very Short Answer', typeKey: 'very_short', badgeColor: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
                        { marks: 2, label: '2 MARKS — Short Answer', typeKey: 'short', badgeColor: 'bg-sky-50 text-sky-700 border-sky-200' },
                        { marks: 3, label: '3 MARKS — Medium Answer', typeKey: 'medium', badgeColor: 'bg-amber-50 text-amber-700 border-amber-200' },
                        { marks: 5, label: '5 MARKS — Long / Analytical Answer', typeKey: 'long', badgeColor: 'bg-purple-50 text-purple-700 border-purple-200' },
                      ].map((group) => {
                        const groupQuestions: ImportantQuestion[] = (result.data as ImportantQuestion[]).filter(
                          q => q.marks === group.marks || q.type === group.typeKey
                        );
                        ...
                        return (
                          <div key={group.marks} className="space-y-4">
                            <div className="flex items-center gap-2 pb-1 border-b border-slate-100">
                              <span className={cn("px-2.5 py-0.5 rounded-md text-[10px] font-extrabold uppercase border tracking-wider", group.badgeColor)}>
                                {group.marks} {group.marks === 1 ? 'Mark' : 'Marks'}
                              </span>
                              <h4 className="font-display font-extrabold text-sm text-slate-900 tracking-tight">
                                {group.label}
                              </h4>
                            </div>
                            ...
                            <div className="space-y-3">
                              {groupQuestions.map((q, qIdx) => {
                                const qKey = `${group.marks}-${qIdx}`;
                                const isExpanded = expandedQuestions[qKey] || false;

                                return (
                                  <div key={qIdx} className="bg-slate-50 border border-slate-200/80 rounded-2xl overflow-hidden shadow-sm transition-all">
                                    <div className="p-4 md:p-5 flex items-start justify-between gap-4">
                                      <div className="space-y-1 flex-1 pr-2">
                                        <div className="flex items-center gap-2">
                                          <span className="text-xs font-bold text-slate-400 font-mono">Q{qIdx + 1}.</span>
                                        ...
                                        <p className="font-bold text-xs md:text-sm text-slate-900 leading-relaxed pt-1">
                                          {cleanMathText(q.question)}
                                        </p>
                                      </div>
                                      <button
                                        onClick={() => setExpandedQuestions(prev => ({ ...prev, [qKey]: !isExpanded }))}
                                        className={cn(
                                          "px-3.5 py-2 rounded-xl text-[10px] font-extrabold uppercase tracking-wider flex items-center gap-1.5 transition-all outline-none border cursor-pointer shrink-0 shadow-sm",
                                          isExpanded
                                            ? "bg-slate-900 border-slate-950 text-white"
                                    ...
                                    {isExpanded && (
                                      <motion.div
                                        initial={{ height: 0, opacity: 0 }}
                                        animate={{ height: 'auto', opacity: 1 }}
                                        className="bg-white border-t border-slate-200/80 p-5 space-y-4"
                                      >
                                        <div className="space-y-2">
                                          <p className="text-[10px] font-bold text-purple-600 uppercase tracking-widest">Model Answer</p>
                                          <div className="markdown-body text-xs text-slate-800 leading-relaxed font-medium bg-purple-50/30 p-4 rounded-xl border border-purple-100">
                                            <ReactMarkdown>{cleanMathText(q.answer)}</ReactMarkdown>
                                          </div>
                                        </div>

                                        {q.keyPoints && q.keyPoints.length > 0 && (
                                          <div className="space-y-2 pt-2 border-t border-slate-100">
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Key Marking Points</p>
                                            <ul className="list-disc pl-5 text-xs text-slate-600 space-y-1 font-medium leading-relaxed">
                                              {q.keyPoints.map((kp, kIdx) => (
                                                <li key={kIdx}>{cleanMathText(kp)}</li>
                                              ))}
                                            </ul>
                                          </div>
                                        )}
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    ...
                    {mcqSubState !== 'config' && (
                      <span className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1 rounded-full">
                        {mcqSubState === 'active' ? `Question ${mcqCurrentIndex + 1} / ${mcqQuestions.length}` : 'Result'}
                      </span>
                    )}
                  </div>
                  ...
                  {qaError && (
                    <div className="p-4 bg-rose-50 border border-rose-200 rounded-2xl space-y-2">
                      <p className="text-xs font-bold text-rose-800">{qaError}</p>
                      <button
                        onClick={handleGenerateMCQs}
                        className="px-3 py-1 bg-rose-600 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider hover:bg-rose-700 transition-colors"
                      >
                        Retry
                      </button>
                    </div>
                  ...
                  {/* CONFIGURATION MODE */}
                  {mcqSubState === 'config' && (
                    <div className="space-y-6 bg-slate-50/70 p-6 rounded-[2rem] border border-slate-200/70">
                      <div>
                        <label className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 block mb-2">Source</label>
                      ...
                      <div>
                        <label className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 block mb-2">Number of Questions</label>
                        <div className="grid grid-cols-4 gap-2">
                          {[5, 10, 15, 20].map((num) => (
                            <button
                              ...
                              className={cn(
                                "py-2.5 rounded-xl font-bold text-xs transition-all border outline-none cursor-pointer",
                                mcqCount === num
                      ...
                      <div>
                        <label className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 block mb-2">Difficulty</label>
                        <div className="grid grid-cols-4 gap-2">
                          {[
                            { id: 'easy', label: 'Easy' },
                            ...
                            <button
                              key={diff.id}
                              type="button"
                              onClick={() => setMcqDifficulty(diff.id as MCQDifficulty)}
                              className={cn(
                                "py-2.5 rounded-xl font-bold text-xs transition-all border outline-none cursor-pointer capitalize",
                                mcqDifficulty === diff.id
                                  ? "bg-teal-600 border-teal-700 text-white shadow-sm"
                                  : "bg-white border-slate-200 text-slate-700 hover:bg-slate-100"
                              )}
                            >
                      ...
                      <button
                        onClick={handleGenerateMCQs}
                        disabled={loading === 'mcq-generator'}
                        className="w-full py-3.5 bg-teal-600 hover:bg-teal-700 text-white font-extrabold text-xs uppercase tracking-wider rounded-xl transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                      >
                  ...
                  {mcqSubState === 'active' && mcqQuestions.length > 0 && (
                    <div className="space-y-6">
                      <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div 
                          className="h-full bg-teal-600 transition-all duration-300"
                          style={{ width: `${((mcqCurrentIndex + 1) / mcqQuestions.length) * 100}%` }}
                        />
                      </div>

                      <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-200/80 space-y-4">
                        <p className="font-display font-semibold text-base text-slate-800 leading-relaxed">
                          {cleanMathText(mcqQuestions[mcqCurrentIndex]?.question || '')}
                        </p>
                      </div>

                      <div className="space-y-3">
                        {mcqQuestions[mcqCurrentIndex]?.options.map((option: string, optIdx: number) => {
                          const isSelected = mcqAnswers[mcqCurrentIndex] === optIdx;
                          ...
                          return (
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                setMcqAnswers(prev => ({ ...prev, [mcqCurrentIndex]: optIdx }));
                              }}
                              className={cn(
                                "w-full text-left p-4 rounded-2xl border font-medium text-sm transition-all flex items-center justify-between group outline-none cursor-pointer",
                                isSelected
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <span className={cn(
                                  "w-6 h-6 rounded-lg text-xs font-bold flex items-center justify-center border shrink-0 transition-colors",
                                  isSelected ? "bg-teal-600 border-teal-600 text-white" : "bg-slate-100 border-slate-200 text-slate-600"
                                )}>
                                  {letter}
                                </span>
                                <span className="flex-1 text-slate-800">{cleanMathText(option)}</span>
                              </div>
                              <div className={cn(
                                "w-5 h-5 rounded-full border flex items-center justify-center shrink-0 transition-all",
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                        <button
                          onClick={() => setMcqCurrentIndex(prev => Math.max(prev - 1, 0))}
                          disabled={mcqCurrentIndex === 0}
                          className="px-4 py-2 text-xs font-bold text-slate-600 hover:bg-slate-100 rounded-xl transition-all disabled:opacity-30 border border-slate-200 outline-none cursor-pointer"
                        >
                          Previous
                        </button>
                        ...
                        {mcqCurrentIndex < mcqQuestions.length - 1 ? (
                          <button
                            onClick={() => setMcqCurrentIndex(prev => Math.min(prev + 1, mcqQuestions.length - 1))}
                            className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl transition-all shadow-sm outline-none cursor-pointer"
                          >
                            Next Question
                          </button>
                        ) : (
                          <button
                            onClick={() => setMcqSubState('result')}
                            className="px-6 py-2.5 bg-teal-600 hover:bg-teal-700 text-white font-extrabold text-xs uppercase tracking-wider rounded-xl transition-all shadow-md outline-none cursor-pointer"
                          >
                            Submit Test
                          </button>
                        )}
                    ...
                    <div className="space-y-6">
                      {(() => {
                        ...
                        return (
                          <>
                            {/* Score Banner */}
                            <div className="p-6 bg-slate-900 text-white rounded-[2rem] text-center space-y-3 shadow-xl">
                              <p className="text-[10px] font-extrabold uppercase tracking-widest text-teal-400">MCQ RESULT</p>
                              <div className="text-4xl font-black tracking-tight font-display">
                                {correctCount} <span className="text-lg font-bold text-slate-400">/ {total}</span>
                              </div>
                              <div className="inline-block px-3 py-1 rounded-full bg-teal-500/20 text-teal-300 font-extrabold text-xs">
                                {percentage}% Score
                              </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                              <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-2xl text-center">
                                <p className="text-[9px] font-extrabold text-emerald-600 uppercase tracking-widest">Correct</p>
                              ...
                              <div className="p-3 bg-rose-50 border border-rose-100 rounded-2xl text-center">
                                <p className="text-[9px] font-extrabold text-rose-600 uppercase tracking-widest">Wrong</p>
                              ...
                              <div className="p-3 bg-slate-50 border border-slate-200 rounded-2xl text-center">
                                <p className="text-[9px] font-extrabold text-slate-500 uppercase tracking-widest">Unanswered</p>
                                <p className="text-lg font-black text-slate-700">{unansweredCount}</p>
                              </div>
                            </div>

                            {/* Question Review List */}
                            <div className="space-y-4 pt-2">
                              <h4 className="font-display font-extrabold text-sm text-slate-900 uppercase tracking-wider">Detailed Review</h4>
                              {mcqQuestions.map((q, idx) => {
                                const selectedOpt = mcqAnswers[idx];
                                const isCorrect = selectedOpt === q.correctAnswer;
                                const isUnanswered = selectedOpt === undefined || selectedOpt === null;

                                return (
                                  <div key={idx} className="p-5 bg-slate-50 border border-slate-200/80 rounded-2xl space-y-3">
                                    <div className="flex items-start justify-between gap-3">
                                      <p className="font-bold text-xs md:text-sm text-slate-900 leading-relaxed">
                                        <span className="text-slate-400 font-mono mr-1">Q{idx + 1}.</span> {cleanMathText(q.question)}
                                      </p>
                                      <span className={cn(
                                        "px-2.5 py-0.5 rounded-md text-[10px] font-extrabold uppercase shrink-0 border",
                                        isCorrect ? "bg-emerald-100 text-emerald-800 border-emerald-200" :
                                        isUnanswered ? "bg-slate-200 text-slate-700 border-slate-300" :
                                      ...
                                      {q.options.map((opt, optIdx) => {
                                        ...
                                        return (
                                          <div
                                            key={optIdx}
                                            className={cn(
                                              "px-3 py-2 rounded-xl text-xs font-medium flex items-center justify-between border",
                                              isRightChoice
                                          ...
                                          >
                                            <span><strong className="mr-1">{letter}.</strong> {cleanMathText(opt)}</span>
                                            {isRightChoice && <span className="text-[10px] font-extrabold uppercase text-emerald-700">Correct Answer</span>}
                                            {!isRightChoice && isUserChoice && <span className="text-[10px] font-extrabold uppercase text-rose-700">Your Answer</span>}
                                          </div>
                                        );
                                      })}
                                    </div>
                                    ...
                                    {q.explanation && (
                                      <div className="p-3 bg-white rounded-xl border border-slate-200 text-xs text-slate-700 leading-relaxed">
                                        <span className="font-bold text-teal-700 block text-[10px] uppercase tracking-wider mb-0.5">Explanation</span>
                                        <ReactMarkdown>{cleanMathText(q.explanation)}</ReactMarkdown>
                                      </div>
                                    )}
                                  </div>
                                );
                            ...
                            <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                              <button
                                ...
                                className="flex-1 py-3 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl transition-all shadow-sm outline-none cursor-pointer"
                              >
                                Try Again
                              </button>
                              <button
                                onClick={() => setMcqSubState('config')}
                                className="flex-1 py-3 bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs rounded-xl transition-all shadow-md outline-none cursor-pointer"
                              >
                                Generate New MCQs
                              </button>
                            </div>
