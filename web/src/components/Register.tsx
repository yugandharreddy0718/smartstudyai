...
import { Sparkles, ArrowRight, AlertCircle, Mail, KeyRound, User, GraduationCap } from 'lucide-react';
import { registerWithEmail } from '@smartstudy/firebase';
...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      await registerWithEmail(email, password, name, grade);
      navigate('/');
    } catch (err: any) {
      console.error(err);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h1 className="font-display font-bold text-3xl text-slate-900 mb-1 tracking-tight">Create Account</h1>
          <p className="text-slate-500 font-medium text-sm">Join SmartStudy AI today</p>
        </div>
        ...
        <div className="bg-slate-50/70 p-5 rounded-2xl border border-slate-200/80 mb-6">
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Class/Grade</label>
              <div className="relative">
                <GraduationCap className="w-5 h-5 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" />
                <select
                  value={grade}
                  onChange={(e) => setGrade(e.target.value)}
                  className="w-full bg-white text-slate-900 pl-12 pr-4 py-3.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 font-medium text-sm shadow-sm appearance-none"
                >
