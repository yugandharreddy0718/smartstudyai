...
import { Home, BookOpen, MessageCircle, User, Upload, Sparkles, ShieldAlert } from 'lucide-react';
...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    ...(showAdminPanel ? [{ icon: ShieldAlert, label: 'Admin Panel', path: '/admin' }] : [])
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
          ...
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) => cn(
                ...
                item.path === '/admin' && !isActive && "text-indigo-600 hover:bg-indigo-50/80"
              )}
            >
              {({ isActive }) => (
                <>
                  <item.icon className={cn("w-5 h-5", isActive ? "text-brand-primary" : item.path === '/admin' ? "text-indigo-600" : "text-slate-400 group-hover:text-slate-900")} />
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-2xl border-t border-slate-100 px-4 py-2 flex justify-around items-center z-50 shadow-[0_-10px_30px_rgba(0,0,0,0.03)]">
        {navItems.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
            ...
            className={({ isActive }) => cn(
              ...
              isActive ? "text-brand-primary scale-105" : "text-slate-400 hover:text-slate-600"
            )}
          >
            {({ isActive }) => (
              <>
                <item.icon className={cn("w-5 h-5 stroke-[2.5]")} />
                <span className="text-[10px] font-bold mt-0.5 tracking-tight">{item.label}</span>
                {isActive && (
                  <motion.div 
                    layoutId="mobile-active"
                    className="absolute -bottom-1 w-1 h-1 bg-brand-primary rounded-full shadow-[0_0_8px_var(--color-brand-primary)]" 
                  />
                )}
              </>
            )}
