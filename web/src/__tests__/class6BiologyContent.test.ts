...
import { CLASS6_BIOLOGY_CONTENT } from '../data/class6BiologyContent';

describe('Phase 6F Class 6 Biology Content Quality Validation', () => {
  ...
  it('1. verifies that exactly 8 Class 6 Biology lessons exist', () => {
    const chapters = getChaptersBySubject('biology', '6');
  ...
  it('2. verifies that all 8 lesson IDs are 100% unique and map to valid chapters', () => {
    ...
    chapters.forEach(ch => {
      ...
      expect(ch.id).toMatch(/^g6-biology-c\d+-l\d+$/);
      expect(ch.chapterId).toMatch(/^g6-biology-c\d+$/);
    });
  ...
  it('6. verifies legacy chapter resolution (e.g. g6-biology-c1) resolves cleanly without 404', () => {
    for (let c = 1; c <= 3; c++) {
      const legacyId = `g6-biology-c${c}`;
  ...
  it('7. verifies student progress tracking with 8 lessons', () => {
    const progressZero = calculateSubjectProgress('biology', [], '6');
    expect(progressZero).toBe(0);

    const progressSome = calculateSubjectProgress('biology', ['g6-biology-c1-l1', 'g6-biology-c1-l2'], '6');
