...
describe('Phase 10 Class 10 Chemistry Quality Validation', () => {
  ...
  it('1. verifies that exactly 11 Class 10 Chemistry lessons exist', () => {
    const chapters = getChaptersBySubject('chemistry', '10');
  ...
  it('2. verifies that all 11 lesson IDs are 100% unique and map to valid chapters', () => {
    ...
    chapters.forEach(ch => {
      ...
      expect(ch.id).toMatch(/^g10-chemistry-c\d+-l\d+$/);
      expect(ch.chapterId).toMatch(/^g10-chemistry-c\d+$/);
    });
  ...
  it('5. verifies legacy chapter resolution (e.g. g10-chemistry-c1) resolves cleanly without 404', () => {
    for (let c = 1; c <= 4; c++) {
      const legacyId = `g10-chemistry-c${c}`;
  ...
  it('6. verifies student progress tracking with 11 lessons', () => {
    const progressZero = calculateSubjectProgress('chemistry', [], '10');
    expect(progressZero).toBe(0);

    const progressSome = calculateSubjectProgress('chemistry', ['g10-chemistry-c1-l1', 'g10-chemistry-c1-l2'], '10');
