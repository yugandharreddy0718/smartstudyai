...
describe('Phase 7 Class 7 Chemistry Quality Validation', () => {
  ...
  it('1. verifies that exactly 5 Class 7 Chemistry lessons exist', () => {
    const chapters = getChaptersBySubject('chemistry', '7');
  ...
  it('2. verifies that all 5 lesson IDs are 100% unique and map to valid chapters', () => {
    ...
    chapters.forEach(ch => {
      ...
      expect(ch.id).toMatch(/^g7-chemistry-c\d+-l\d+$/);
      expect(ch.chapterId).toMatch(/^g7-chemistry-c\d+$/);
    });
  ...
  it('5. verifies legacy chapter resolution (e.g. g7-chemistry-c1) resolves cleanly without 404', () => {
    for (let c = 1; c <= 2; c++) {
      const legacyId = `g7-chemistry-c${c}`;
  ...
  it('6. verifies student progress tracking with 5 lessons', () => {
    const progressZero = calculateSubjectProgress('chemistry', [], '7');
    expect(progressZero).toBe(0);

    const progressSome = calculateSubjectProgress('chemistry', ['g7-chemistry-c1-l1', 'g7-chemistry-c1-l2'], '7');
