...
describe('Phase 7 Class 7 Geography Quality Validation', () => {
  ...
  it('1. verifies that exactly 13 Class 7 Geography lessons exist', () => {
    const chapters = getChaptersBySubject('geography', '7');
  ...
  it('2. verifies that all 13 lesson IDs are 100% unique and map to valid chapters', () => {
    ...
    chapters.forEach(ch => {
      ...
      expect(ch.id).toMatch(/^g7-geography-c\d+-l\d+$/);
      expect(ch.chapterId).toMatch(/^g7-geography-c\d+$/);
    });
  ...
  it('5. verifies legacy chapter resolution (e.g. g7-geography-c1) resolves cleanly without 404', () => {
    for (let c = 1; c <= 6; c++) {
      const legacyId = `g7-geography-c${c}`;
  ...
  it('6. verifies student progress tracking with 13 lessons', () => {
    const progressZero = calculateSubjectProgress('geography', [], '7');
    expect(progressZero).toBe(0);

    const progressSome = calculateSubjectProgress('geography', ['g7-geography-c1-l1', 'g7-geography-c1-l2'], '7');
