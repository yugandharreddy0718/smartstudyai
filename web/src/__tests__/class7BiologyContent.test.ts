...
describe('Phase 7 Class 7 Biology Quality Validation', () => {
  ...
  it('1. verifies that exactly 10 Class 7 Biology lessons exist', () => {
    const chapters = getChaptersBySubject('biology', '7');
  ...
  it('2. verifies that all 10 lesson IDs are 100% unique and map to valid chapters', () => {
    ...
    chapters.forEach(ch => {
      ...
      expect(ch.id).toMatch(/^g7-biology-c\d+-l\d+$/);
      expect(ch.chapterId).toMatch(/^g7-biology-c\d+$/);
    });
  ...
  it('5. verifies legacy chapter resolution (e.g. g7-biology-c1) resolves cleanly without 404', () => {
    for (let c = 1; c <= 4; c++) {
      const legacyId = `g7-biology-c${c}`;
  ...
  it('6. verifies student progress tracking with 10 lessons', () => {
    const progressZero = calculateSubjectProgress('biology', [], '7');
    expect(progressZero).toBe(0);

    const progressSome = calculateSubjectProgress('biology', ['g7-biology-c1-l1', 'g7-biology-c1-l2'], '7');
