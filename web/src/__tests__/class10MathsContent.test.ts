...
describe('Phase 10 Class 10 Mathematics Quality Validation', () => {
  ...
  it('1. verifies that exactly 37 Class 10 Maths lessons exist', () => {
    const chapters = getChaptersBySubject('maths', '10');
  ...
  it('2. verifies that all 37 lesson IDs are 100% unique and map to valid chapters', () => {
    ...
    chapters.forEach(ch => {
      ...
      expect(ch.id).toMatch(/^g10-maths-c\d+-l\d+$/);
      expect(ch.chapterId).toMatch(/^g10-maths-c\d+$/);
    });
  ...
  it('5. verifies legacy chapter resolution (e.g. g10-maths-c1) resolves cleanly without 404', () => {
    for (let c = 1; c <= 14; c++) {
      const legacyId = `g10-maths-c${c}`;
  ...
  it('6. verifies student progress tracking with 37 lessons', () => {
    const progressZero = calculateSubjectProgress('maths', [], '10');
    expect(progressZero).toBe(0);

    const progressSome = calculateSubjectProgress('maths', ['g10-maths-c1-l1', 'g10-maths-c1-l2'], '10');
