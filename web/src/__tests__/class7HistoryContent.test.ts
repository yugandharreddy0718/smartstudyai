...
describe('Phase 7 Class 7 History Quality Validation', () => {
  ...
  it('1. verifies that exactly 15 Class 7 History lessons exist', () => {
    const chapters = getChaptersBySubject('history', '7');
  ...
  it('2. verifies that all 15 lesson IDs are 100% unique and map to valid chapters', () => {
    ...
    chapters.forEach(ch => {
      ...
      expect(ch.id).toMatch(/^g7-history-c\d+-l\d+$/);
      expect(ch.chapterId).toMatch(/^g7-history-c\d+$/);
    });
  ...
  it('5. verifies legacy chapter resolution (e.g. g7-history-c1) resolves cleanly without 404', () => {
    for (let c = 1; c <= 7; c++) {
      const legacyId = `g7-history-c${c}`;
  ...
  it('6. verifies student progress tracking with 15 lessons', () => {
    const progressZero = calculateSubjectProgress('history', [], '7');
    expect(progressZero).toBe(0);

    const progressSome = calculateSubjectProgress('history', ['g7-history-c1-l1', 'g7-history-c1-l2'], '7');
