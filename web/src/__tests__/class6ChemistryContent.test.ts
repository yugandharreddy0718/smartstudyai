...
import { CLASS6_CHEMISTRY_CONTENT } from '../data/class6ChemistryContent';

describe('Phase 6E Class 6 Chemistry Content Quality Validation', () => {
  ...
  it('1. verifies that exactly 5 Class 6 Chemistry lessons exist', () => {
    const chapters = getChaptersBySubject('chemistry', '6');
  ...
  it('2. verifies that all 5 lesson IDs are 100% unique and map to valid chapters', () => {
    ...
    chapters.forEach(ch => {
      ...
      expect(ch.id).toMatch(/^g6-chemistry-c\d+-l\d+$/);
      expect(ch.chapterId).toMatch(/^g6-chemistry-c\d+$/);
    });
  ...
  it('5. verifies word counts are within academic range (600 - 900 words)', () => {
    ...
    const avgWords = Math.round(totalWords / 5);
    expect(avgWords).toBeGreaterThanOrEqual(500);
  });

  it('6. verifies legacy chapter resolution (e.g. g6-chemistry-c1) resolves cleanly without 404', () => {
    for (let c = 1; c <= 2; c++) {
      const legacyId = `g6-chemistry-c${c}`;
  ...
  it('7. verifies student progress tracking with 5 lessons', () => {
    const progressZero = calculateSubjectProgress('chemistry', [], '6');
    expect(progressZero).toBe(0);

    const progressSome = calculateSubjectProgress('chemistry', ['g6-chemistry-c1-l1', 'g6-chemistry-c1-l2'], '6');
