...
describe('Phase 9 Class 9 Physics Quality Validation', () => {
  ...
  it('1. verifies that exactly 11 Class 9 Physics lessons exist', () => {
    const chapters = getChaptersBySubject('physics', '9');
  ...
  it('2. verifies that all 11 lesson IDs are 100% unique and map to valid chapters', () => {
    ...
    chapters.forEach(ch => {
      ...
      expect(ch.id).toMatch(/^g9-physics-c\d+-l\d+$/);
      expect(ch.chapterId).toMatch(/^g9-physics-c\d+$/);
    });
  ...
  it('5. verifies legacy chapter resolution (e.g. g9-physics-c1) resolves cleanly without 404', () => {
    for (let c = 1; c <= 4; c++) {
      const legacyId = `g9-physics-c${c}`;
  ...
  it('6. verifies student progress tracking with 11 lessons', () => {
    const progressZero = calculateSubjectProgress('physics', [], '9');
    expect(progressZero).toBe(0);

    const progressSome = calculateSubjectProgress('physics', ['g9-physics-c1-l1', 'g9-physics-c1-l2'], '9');
